...
.directive('slider', ['$timeout', function($timeout){
	...
	return {
		...
		template: '<div class="sliders">'
		        ...
		        + '<a ng-href="{{item.href}}">'
		        + '<img ng-src="{{item.src}}">'
            + '<h3 ng-bind="item.title" style="position: absolute;bottom: 0px;width: 100%;text-align: right;background-color: rgba(0,0,0,0.3);margin: 0px;padding: 10px">'
            +'</h3>'
		        + '</a>'
		        + '</li>'
		        + '</ul>'
